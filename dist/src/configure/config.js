 ...
 */
async function load(defaults = contstants_1.CONFIG_DEFAULTS) {
    let { data: config } = await helpers_1.promise(readRoot());
    if (!config) {
        log_1.log.alert(`Failed to lookup Gobu config in package.json or "gobu.json".`);
        return null;
    }
 ...
 */
function extendCommands(config, pargs, cli) {
    const entrypoints = [];
...
exports.extendCommands = extendCommands;
